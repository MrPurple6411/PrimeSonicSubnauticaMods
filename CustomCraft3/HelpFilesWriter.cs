...
namespace CustomCraft3
{
    ...
    internal static partial class HelpFilesWriter
    {
        ...
        private static void GenerateOriginalRecipes()
        {
            ...
            Dictionary<TechGroup, Dictionary<TechCategory, List<TechType>>> allGroups = CraftData.groups;
            ...
            GenerateOriginalCraftingTabPaths();
            GenerateOriginalCraftingNodePaths();
        ...
        private static void GenerateOriginalBioFuels()
        {
            ...
            Dictionary<TechType, float> allBioFuels = BaseBioReactor.charge;
            ...
            {
            "This file was generated with current BioFuel energy values in the game",
        ...
        {
            const string fileName = "CraftingTabs.txt";
            string path = Path.Combine(FileLocations.OriginalsFolder, fileName);
            ...
            printyPrints.AddRange(EmUtils.CommentTextLinesCentered(new string[]
            {
            "This file was generated with current CraftTree.Type values in the game",
            "You can copy individual entries from this file to use in your personal overrides",
            "--------------------------------------------------------------------------------",
            "",
            "",
            ...
            {
                CraftTree craftTree = CraftTree.GetTree(type);

                if (craftTree is null)
                    continue;

                printyPrints.AddRange(EmUtils.CommentTextLinesCentered(new string[]
                {
                            $"# {Language.main.Get(craftTree.id)} #",
            "--------------------------------------------------------------------------------",
                }));
                printyPrints.Add("");
                printyPrints.AddRange(RecursivelyGenerateCraftNodes(craftTree.nodes, craftTree.id));
                printyPrints.Add("");
                printyPrints.Add("");
            }

            File.WriteAllLines(path, printyPrints, Encoding.UTF8);

            QuickLogger.Debug($"{fileName} generated with current values.");
        }

        private static IEnumerable<string> RecursivelyGeneratePaths(CraftNode nodes, string currentPath)
        {
            var printyPrints = new List<string>();

            foreach (CraftNode node in nodes.nodes)
            {
                string newPath = $"{currentPath}/{node.id}";
                if (node.action == TreeAction.Expand)
                {
                    printyPrints.Add(newPath);
                    printyPrints.AddRange(RecursivelyGeneratePaths(node, newPath));
                    continue;
                }
            }
        ...
        private static void GenerateOriginalCraftingNodePaths()
        {
            const string fileName = "CraftingNodes.txt";
            string path = Path.Combine(FileLocations.OriginalsFolder, fileName);
        ...
        private static IEnumerable<string> RecursivelyGenerateCraftNodes(CraftNode nodes, string currentPath)
        {
            var printyPrints = new List<string>();
            ...
            foreach (CraftNode node in nodes.nodes)
            {
                ...
                if (node.action == TreeAction.Expand)
                {
                    printyPrints.AddRange(RecursivelyGenerateCraftNodes(node, newPath));
                    continue;
                }

                int pathLength = newPath.Length;

                // Add padding to end of path and then add the Language line for the id.
                newPath = $"{newPath}{new string(' ', 100 - pathLength)}# {Language.main.Get(node.id)} #";
                printyPrints.Add(newPath);
            }
        ...
        private static void GenerateOriginalsFile(TechGroup group, TechCategory category, List<TechType> list, string fileName)
        {
            ...
            printyPrints.AddRange(EmUtils.CommentTextLinesCentered(new string[]
            {
            "This file was generated with the current recipes in the game",
            "You can copy individual entries from this file to use in your personal overrides",
            "--------------------------------------------------------------------------------",
